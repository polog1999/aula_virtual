...
class AsistenciaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('disciplina', 'periodo', 'sede', 'docente', 'fecha', 'seccion')) {
            ...
            if ($asistencias->isNotEmpty()) {
                return view('portal.buscarAsistencia', compact('disciplina1', 'seccion', 'matriculas', 'periodos', 'asistencias', 'disciplinaSeleccionada', 'info'));
            } else {
                //  if ($asistencias == null) {
    ...
    public function store(Request $request)
    {
        ...
        foreach ($asistencias as $matriculaId => $asistencia) {
            ...
            $asistencias1 = Asistencia::updateOrCreate(
                ...
                [
                    'estado' => $asistencia,
                    'detalles' => $request->observaciones[$matriculaId] ?? null

                ]
                    
            );
            // dd($request->observaciones[$matriculaId]);
        }
        // dd($asistencias);
        return redirect()
            ->back()
        ...
        foreach ($asistencias as $matriculaId => $asistencia) {
            $a = Asistencia::where('matricula_id', $matriculaId)
                ->where('fecha', $request->fecha)->update([
                    'estado' => $asistencia,
                    'detalles' => $request->observaciones[$matriculaId]
                ]);
        }
    ...
    public function getDisciplinas($periodoId)
    {
        //disciplinas = DB::table('secciones')

        $disciplinas = DB::table('secciones')
            ->join('cursos', 'cursos.id', '=', 'secciones.curso_id')
    ...
    public function getDocentes($periodoId, $disciplinaId)
    {
        $secciones = Seccion::where('periodo_id', $periodoId)
            // ->where('lugar_id', $lugarId)
          ->where('curso_id', $disciplinaId)
           
            ->with('docentes.user:id,nombres,apellido_paterno,apellido_materno')
            ->select('id', 'curso_id', 'docente_id')
            ->get();
        // dd($secciones);
    ...
    public function getSecciones($periodoId, $disciplinaId, $docenteId)
    {
    ...
    public function getSesiones($sesionId)
    {
        $sesiones =  Sesion::where('seccion_id', $sesionId)
            ->orderBy('fecha', 'asc')
            ->pluck('fecha')
            ->map(function ($sesion) {
                return $sesion->format('d-m-Y');
            });
        // dd($sesiones->toArray());
    ...
    public function reporte(Request $request)
    {
        ...
        if ($periodoId && $disciplinaId && $seccionId && $mes) {
            ...
            $diasDelMes = Sesion::where('seccion_id', $seccionId)->whereMonth('fecha', $mes)->pluck('fecha');
            // dd($diasDelMes->toArray());
        ...
        return view('portal.reporteAsistencia', compact(
            'periodos',
            'disciplinas',
            'estudiantesReporte',
            'diasDelMes',
    ...
    {
        $periodos = Periodo::select('id', 'anio', 'ciclo')->get();
        return view('portal.reporteExcelAsistencias', compact('periodos'));
    }
}
